<?php 
  ...
  $html = '<table class="table" style="font-size: 13px ; font-family: Calibri; border:2px solid #000;  background: url('.URL_IMG.'/logo2.png); background-position: center; padding: 10px; width: 100%; height:3308px; text-transform: uppercase;">
      ...
      ///CONTENIDO DE RESPONSIVA
      $html2='';
      $html2 .= '<table class="table" style="font-size: 14px ; font-family: Calibri; border:2px solid #000;  background: url('.URL_IMG.'/logo2.png); background-position: center; padding: 10px; width: 100%; height:3308px; ">
          <thead>
            ...
            <tr>
              ...
              <p style="padding:3px; font-weight: normal; text-align: left !important; padding-botom:10px;">
                ...
                El suscrito <span style="text-decoration: underline ; ">' . $_POST['name_driver'] . '</span>  
                 conductor del tracto camión marca: <span style="text-decoration: underline  ;"> ' . $_POST['brand'] .' '. $_POST['model'] . '</span>  Placas: <span style="text-decoration: underline ;">' . $_POST['plates_t'] . '</span> que arrastra la caja '. $_POST['box'].' con placas <span style="text-decoration: underline  ;">' . $_POST['plates'] . '</span>  propiedades de <span style="text-decoration: underline  ;">' . $_POST['name_trasport'] . '</span> hago constar que he recibido e inspeccionado la carga contenida en la caja <span style="text-decoration: underline  ;"> '. $_POST['box'].' </span> antes mencionada, la cual contiene exclusivamente:
          ...
          <tbody>
            ...
            <tr width="100%">
              ...
              <p style="width:100%; padding: 5px; font-weight: normal; text-align: justify !important;">
                ...
                <span style="text-decoration: underline ; "> '.$_POST['temperature'].' - '.$_POST['degrees'].'</span> 
                con salida el día 
                <span style="text-decoration: underline ; ">'.$_POST['date'].'</span> a las 
                <span style="text-decoration: underline ; ">'.$_POST['time'].'</span>, para llegar a la <br> ciudad de: <span style="text-decoration: underline;"> ' . $_POST['city'] . ' </span> mas tardar el día de: <span style="text-decoration: underline ; ">'.$tomorrow.'</span>. <br>
                En caso de no llegar al almacen de la fecha indicada, se descontara un 20% del flete.
                La que será conforma a la guía y remisión de embarque <span style="text-decoration: underline ; "> '.$_POST['f_embark'].'</span>. Por este conducto manifiesto haber estado presente y por lo tanto tener pleno conocimiento de <span style="text-decoration: underline ; "> '.$totalunit.'</span> cajas de producto antes mencionado, durante y hasta el momento de cerrar las puertas del camión a mi cargo correspondiente a este embarque. <br>
                ...
                <table width="100%">
                  ...
                  <tr>
                    <th width="100%" style="text-align: left; border-top: 1px solid #ccc; padding-top: 10px;">
                      ...
                      <br>  
                      <label for="" style=" font-size: 12px;">NOMBRE:</label> <span style="font-weight: normal;">' . $_POST['name_driver'] . '</span>
                      <br>  
                      <label for="" style=" font-size: 12px;">TELEFONO:</label> <span style="font-weight: normal;">' . $_POST['phone_driver'] . '</span>
                    </th>
                  </tr>
                </table>
              </th>
    ...
    ////SALIDA DE PDF DE REMISION MODIFICADA
    $mpdf = new Mpdf();
    $mpdf->WriteHTML($html);
    $mpdf->Output(dirname(__FILE__).'/../pdf_files/referrals/EDIT_REM_'.$_POST['f_charge'].'.pdf','F');


    ///SALIDA DE PDF RESPONSIVA MODIFICADA
    $mpdf2 = new Mpdf();
    $mpdf2->WriteHTML($html2);
    ob_clean();
    $mpdf2->Output(dirname(__FILE__).'/../pdf_files/responsives/EDIT_RES_'.$_POST['f_charge'].'.pdf','F');
